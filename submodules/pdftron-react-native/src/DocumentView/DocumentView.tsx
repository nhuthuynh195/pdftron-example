...
const propTypes = {
  ...
  onMoreButtonPressed: func<() => void>(),
  onCommentHistoryPressed: func<() => void>(),
  onNotesHistoryPressed: func<() => void>(),
  // end
...
function func<T>(): Requireable<T> {
  let validator: Validator<T> = function (
...
export class DocumentView extends PureComponent<DocumentViewProps, any> {
  ...
  getAllFields = (
    pageNumber?: number,
  ): Promise<void | Array<AnnotOptions.Field>> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
  ...
  saveSignature = (xfdf: string): Promise<void | string> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
  ...
  removeSignature = (index: number): Promise<void | string> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
  ...
  removeAllSignature = (): Promise<void | string> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
   ...
   */
  setValueForFields = (
    fieldsMap: Record<string, string | boolean | number>,
  ): Promise<void> => {
  ...
  setFlagForAnnotations = (
    annotationFlagList: Array<AnnotOptions.AnnotationFlag>,
  ): Promise<void> => {
  ...
  setPropertyForAnnotation = (
    id: string,
    pageNumber: number,
    propertyMap: AnnotOptions.AnnotationProperties,
  ): Promise<void> => {
  ...
  ): Promise<void | AnnotOptions.Annotation> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
  ...
  getPageCropBox = (
    pageNumber: number,
  ): Promise<void | AnnotOptions.CropBox> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
  ...
  convertScreenPointsToPagePoints = (
    points: Array<AnnotOptions.Point>,
  ...
  setColorPostProcessColors = (
    whiteColor: AnnotOptions.Color,
    blackColor: AnnotOptions.Color,
  ): Promise<void> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
  ...
  getSelection = (
    pageNumber: number,
  ): Promise<void | AnnotOptions.TextSelectionResult> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
  ...
  getSelectionPageRange = (): Promise<void | {
    begin: number
    end: number
  }> => {
    const tag = findNodeHandle(this._viewerRef)
  ...
  setDefaultPageColor = (
    defaultPageColor: AnnotOptions.Color,
  ): Promise<void> => {
    const tag = findNodeHandle(this._viewerRef)
    if (tag != null) {
